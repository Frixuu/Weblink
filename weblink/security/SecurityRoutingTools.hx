...
final class SecurityRoutingTools {
	 ...
	 */
	public static inline function jwks<T:IHttpRouter>(router:T, jwks:Jwks, path = "/jwks"):T {
	...
	public static inline function users<T:IHttpRouter>(router:T, credentialsProvider:CredentialsProvider, path = "/users"):T {
		router.get(path, credentialsProvider.getUsersEndpoint);
		router.post(path, credentialsProvider.postUsersEndpoint);
		return router;
	}

	public static inline function oauth2<T:IHttpRouter>(router:T, secret_key:String, credentialsProvider:CredentialsProvider, path = "/token"):T {
